...
from esculator.tests.data import (
    ...
    ANNUAL_COSTS_REDUCTION, BIDS, DISCOUNT_RATES
)
...
class AnnualCostsReduction(TestCalculationsMixin, unittest.TestCase):
    __test__ = True
    field_name = 'annualCostsReduction'
    test_data = ANNUAL_COSTS_REDUCTION
    bid = deepcopy(BASE_BID)


class NBUdiscountRate(TestCalculationsMixin, unittest.TestCase):
    __test__ = True
    field_name = 'NBUdiscountRate'
    test_data = DISCOUNT_RATES
    bid = deepcopy(BASE_BID)
